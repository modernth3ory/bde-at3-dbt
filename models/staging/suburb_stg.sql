...
-------------------------------------------------------------------#
-- Staging | This model is used to stage the suburb data from the raw schema.
-------------------------------------------------------------------#
...
-- Step 0: Define the staging model for the suburb data.
-------------------------------------------------------------------#
...
-- Step 1: Transformation for suburb data.
-- (Removing leading and trailing whitespace, and capitalising.)
-------------------------------------------------------------------#

WITH transformed_suburb AS (
    SELECT
        TRIM(BOTH ' ' FROM INITCAP(lga_name)) AS lga_name, 
        TRIM(BOTH ' ' FROM INITCAP(suburb_name)) AS suburb_name,
        ingestion_datetime
    FROM raw.suburb
)
